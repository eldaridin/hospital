...
public class PharmacyServiceTest {
    ...
    @Test
    void saveMedication_DeberiaGuardar_CuandoCodigoNoExiste() {
        ...
        Medication resultado = service.saveMedication(medicationReal);
    ...
    @Test
    void saveMedication_DeberiaLanzarIllegalArgumentException_CuandoCodigoExiste() {
        when(repository.findByCode("PAR-01")).thenReturn(Optional.of(medicationReal));

        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class,
                () -> service.saveMedication(medicationReal));

        assertEquals("El código del medicamento ya está registrado.", exception.getMessage());
        verify(repository, times(1)).findByCode("PAR-01");
    ...
    void dispenseMedication_DeberiaReducirStockYGuardar_CuandoHaySuficiente() {
        ...
        assertNotNull(resultado);
        assertEquals(80, resultado.getStock()); // 100 - 20 = 80
        verify(repository, times(1)).findByCode("PAR-01");
        verify(repository, times(1)).save(medicationReal);
    }
    ...
    void dispenseMedication_DeberiaLanzarRuntimeException_CuandoNoExiste() {
        ...
        assertEquals("Medicamento no encontrado con código: DESC-99", exception.getMessage());
